// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

package com.azure.provisioning.storage.generated;

import com.azure.provisioning.storage.generated.models.ManagementPolicyProperties;
import com.azure.provisioning.BicepValue;
import com.azure.provisioning.primitives.Resource;
import com.azure.provisioning.tmp.ResourceType;

public class ManagementPolicyResource extends Resource {

    private final BicepValue<ManagementPolicyProperties> properties;
    private final BicepValue<String> name;

    public ManagementPolicyResource(String identifierName) {
        this(identifierName, null);
    }

    public ManagementPolicyResource(String identifierName, String resourceVersion) {
        super(identifierName, new ResourceType("Microsoft.Storage/storageAccounts/managementPolicies"), resourceVersion);
        properties = BicepValue.defineProperty(this, "properties", new String[] { "properties" }, false, false, false, null);
        name = BicepValue.defineProperty(this, "name", new String[] { "name" }, false, false, false, null);
    }

    public BicepValue<ManagementPolicyProperties> getProperties() {
        return this.properties;
    }

    public ManagementPolicyResource setProperties(BicepValue<ManagementPolicyProperties> properties) {
        this.properties.assign(properties);
        return this;
    }

    public ManagementPolicyResource setProperties(ManagementPolicyProperties properties) {
        return this.setProperties(BicepValue.from(properties));
    }

    public BicepValue<String> getName() {
        return this.name;
    }

    public ManagementPolicyResource setName(BicepValue<String> name) {
        this.name.assign(name);
        return this;
    }

    public ManagementPolicyResource setName(String name) {
        return this.setName(BicepValue.from(name));
    }

}
